...
public class UITalent : BaseDialog
{
    ...
    private void RefreshTalnetShow()
    {
        ...
        talentPoint.text = "剩余天赋点数: " + SaveSlotData.Instance.GetTalentPoint().ToString();
        unlockButton.gameObject.SetActive(!SaveSlotData.Instance.CheckTalent(currTalent.key));
        unlockButton.interactable = SaveSlotData.Instance.CanUnlockTalent(currTalent.key);
    }
}
