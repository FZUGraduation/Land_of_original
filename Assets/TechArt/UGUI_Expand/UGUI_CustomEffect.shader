    ...
    SubShader
    {
		...
		HLSLINCLUDE
            ...
            half4 Outline(half4 color, half4 outlineColor, float width, float2 uv, float4 originalUV)
            {
                ...
                //让描边变色
                 half4 result = lerp(color,outlineColor,outlineRange);
		     ...
		     half4 ShadowFragment(Varyings i) : SV_Target
            {
                 ...
                 //剔除原UV之外内容
                    ...
                    half4 shadow = half4(_ShadowColor.rgb,mainTex_shadow);
                        ...
                        //剔除原UV之外内容
                        float outlineWidth = _OutlineWidth+0.0001f;
                        shadow = Outline(shadow,shadow,outlineWidth,shadowUV,shadowOriginalUV);
                        #endif
                    return shadow;
                #else
                    discard;
